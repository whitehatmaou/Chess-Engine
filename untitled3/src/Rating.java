public class Rating {
    ...
    public static int rateAttack() {
        int counter=0;
        int tempPositionC=Main.kingPositionC;
        for (int i=0;i<64;i++) {
            switch (Main.chessBoard[i/8][i%8]) {
                case "P": {Main.kingPositionC=i; if (!Main.kingSafe()) {counter-=64;}}
                break;
                case "R": {Main.kingPositionC=i; if (!Main.kingSafe()) {counter-=500;}}
                break;
                case "K": {Main.kingPositionC=i; if (!Main.kingSafe()) {counter-=300;}}
                break;
                case "B": {Main.kingPositionC=i; if (!Main.kingSafe()) {counter-=300;}}
                break;
                case "Q": {Main.kingPositionC=i; if (!Main.kingSafe()) {counter-=900;}}
                break;
            }
        }
        Main.kingPositionC=tempPositionC;
        if (!Main.kingSafe()) {counter-=200;}
        return counter/2;
    }
        ...
        for (int i=0;i<64;i++) {
            switch (Main.chessBoard[i/8][i%8]) {
                ...
                case "Q": counter+=queenBoard[i/8][i%8];
                    break;
                case "A": if (material>=1750) {counter+=kingMidBoard[i/8][i%8]; counter+=Main.posibleA(Main.kingPositionC).length()*10;} else
                {counter+=kingEndBoard[i/8][i%8]; counter+=Main.posibleA(Main.kingPositionC).length()*30;}
                    break;
            }
        }
        return counter;
